...
import com.fyxridd.lib.items.model.GetInfo;
import com.fyxridd.lib.items.model.GetInfo.GetItem;
...
public class ItemsManager{
	//插件名 获取类型名 获取类型
	private static Map<String, Map<String, GetInfo>> gets = new HashMap<>();
	//插件名 文件名 物品类型名 物品类型
	private static Map<String, Map<String, Map<String, ItemInfo>>> items = new HashMap<>();
	...
	public List<ItemStack> getItems(String plugin, String type) {
        ...
        GetInfo getInfo;
        try {
            getInfo = gets.get(plugin).get(type);
            if (getInfo == null) return result;
        } catch (Exception e) {
            return result;
        }
		for (GetItem getItem:getInfo.getList()) {
			ItemInfo itemInfo = getItem.getItemInfo();
			if (itemInfo != null && !itemInfo.getItemList().isEmpty()) {//不为null且有内容
				if (getItem.getMethod() == 1) {//方式一
					int times = MathApi.nextInt(0, getItem.getMaxTimes()-getItem.getMinTimes())+getItem.getMinTimes();
					if (getItem.isAll()) {//可重复取
					...
					for (ItemWrapper iw:itemInfo.getItemList()) {
						if (!must && left <= getItem.getMinAmount()-sum) must = true;
						left --;
						if (sum >= getItem.getMaxAmount()) break;//已经达到最大数量
						int chance = itemInfo.getItemList().getChance(iw);
						if (must || (chance > 0 && MathApi.nextInt(0, getItem.getMaxChance()-1) < chance)) {
							sum ++;
                            ItemStack is = iw.getItem();
                            if (is != null) result.add(is);
						}
	...
	public ItemInfo getItemInfo(String plugin, String file, String type) {
		try {
			return items.get(plugin).get(file).get(type);
		} catch (Exception e) {
			return null;
		}
	}
	...
	private void loadItemInfo(String plugin) {
		//物品类型保存的文件夹路径
		String itemPath = CoreApi.pluginPath+File.separator+plugin+File.separator+"items";
		File dir = new File(itemPath);
		dir.mkdirs();
		//清空旧的
		items.remove(plugin);
		//读取新的
		Map<String, Map<String, ItemInfo>> fileHash = new HashMap<>();
		for (File file:dir.listFiles()) {
			if (file.isFile() && file.canRead() && file.getName().endsWith(".yml")) {
				...
				if (!fileName.isEmpty()) {
					Map<String,ItemInfo> typeHash = loadItemInfo(plugin, file);
					if (!typeHash.isEmpty()) fileHash.put(fileName, typeHash);
				}
			}
		}
		if (!fileHash.isEmpty()) {
			items.put(plugin, fileHash);
			//更新生成ItemWrappers
            for (Map<String, ItemInfo> typeHash:fileHash.values()) {
	 ...
	 * @return 异常返回空的hash
	 */
	private Map<String, ItemInfo> loadItemInfo(String plugin, File file) {
        String fileName = file.getName().substring(0, file.getName().length()-4).trim();
		Map<String,ItemInfo> typeHash = new HashMap<>();
		try {
			YamlConfiguration config = UtilApi.loadConfigByUTF8(file);
	...
	private ItemInfo loadItemInfo(String plugin, String file, String type) {
		try {
			...
			loading.add(check);
			//
			String itemPath = CoreApi.pluginPath+File.separator+plugin+File.separator+"items"+File.separator+file+".yml";
			YamlConfiguration config = UtilApi.loadConfigByUTF8(new File(itemPath));
			//方式判断
	...
	private void loadGetInfo(String plugin, ConfigurationSection cs) {
		...
		if (!hash.isEmpty()) gets.put(plugin, hash);
	}
